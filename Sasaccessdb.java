...
public class Sasaccessdb {
    ...
    public static String numbatch, idBatch, datebatch, statebatch, checkbatch, bbatch, sapbatch;  // vars for db query
    public static String batchNum, batchId, sentDate, responseDate, state,t1, t2, check, piece, sap, status, batch, res, running, ship, nowSas, checkStatus;  // for Mailers db table
    ...
    public static String cid, station, employee, company, location, department, cphone, cdid, cemail, cname, sapid, nextcloudu, nextcloudp, chostname, cmac, rocketu, rocketp, bitwardenu, bitwardenp, cip, serial, pin, puk, mkey, duo;  // for computer db
    ...
    //                                                                        Computer Print
    // ============================================================================================================================================================
    public void printComputer(String cid, String station, String employee, String company, String location, String department, String cphone, String cdid, 
    String cemail, String cname, String sapid, String nextcloudu, String nextcloudp, String chostname, String cmac, String rocketu, String rocketp, String bitwardenu, String bitwardenp, String cip,
    String serial, String pin, String puk, String mkey, String duo){
        String devider = "\n\t ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\t ";
        String sappass;
        if(sapid.equals("---")){ sappass = "---";} else{sappass = "12345";}
        if(cid.length() == 1  ){ cid = "0" + cid; } 
        cdid = cdid.replace(".",""); cdid = cdid.replace("E9","");
        station = station.replace(".0","");
        if(station.length() == 1){station = "0" + station;}
        if(bitwardenu.equals("---")){
            bitwardenu = "";
            for(int i = 0; i < rocketu.length(); i++){
                bitwardenu = bitwardenu + "-";
            }
        }
        if(cemail.contains("#")){
            int ja = cemail.indexOf("#");
            String[] email2 = cemail.split("",cemail.length());
            String newEmail = "";;
            for(int i = 0; i < ja; i++){
                newEmail = newEmail + email2[i];
            }
            cemail = newEmail;
        }
        ...
        if(files[0].equals("ALL")){
        ...
        else if(files[0].contains("NAME>")){
            System.out.println("\n\t[" + cid + "] " + employee);
        }
        else if(files[0].contains("LOCATION")){
            System.out.println("\n\t[" + cid + "] " + station + "\t" + employee);
        }
        else{
            System.out.println("\n\t.========.=============================================================================.\n\t|        |  ext: " +cphone.replace(".0","") + 
            "                                                                   |\n\t|        |" + 
            "                                                                             |" +
            "\n\t|   "+ station + "   |  ID\tComputer Name\tHostname\tMac Address\t\tIP Address     |" +
            "\n\t|        |  -------------------------------------------------------------------------- |" +
            "\n\t|        |  " + cid + "\t" + cname +"\t"+ chostname +"\t"+ cmac +"\t"+ cip + "    |" +
            "\n\t'========'============================================================================='" +  B+B+  
            ...
            "\n\t Email" + devider + cemail + devider + 
            "\n\t SAP" + devider + sapid + " | " + sappass + devider +
            "\n\t NextCloud" + devider + nextcloudu + " | " + nextcloudp + devider +
            "\n\t Rocket Chat" + devider + rocketu + " | " + rocketp + devider +
            "\n\t Bitwarden" + devider + bitwardenu + " | " + bitwardenp + devider
            );
        }
    ...
    public void printRes(String batchNum, String batchId, String sentDate, String state, String t1, String t2, String check, String piece, String sap, String status, String batch){
        ...
        if(files[0].equals("ALL")){
            System.out.println( "\t" + batch + "  [" + batchNum + "] " + batchId );
        }
            ...
            if(status.equals(" [ . . . S h i p p e d ]") || status.equals("shipped")){ship = " x ";}else{ship="";}
            if(sentDate.length() < 8){
                System.out.print( "\n\t\t\t\t\t\t[" + batchNum + "] " + batchId );
            } else{
                System.out.print( 
            ...
            if(status.equals("printed")){
                if(sentDate.equals(newdate)){
                    System.out.println( "\t  now > [" + batchNum + "] " + batchId );
                }
                else{
    ...
    public void vars(String sasDb, String sas){
        ...
        try { 
            ...
            else if(sasDb.equals("Computer")){
                while(rs.next()) {   
                            if(rs.getString(1) == null){cid = "";} else{cid = rs.getString(1);}                                             
                            if(rs.getString(2) == null){station = "--";} else{station = rs.getString(2);}
                            if(rs.getString(3) == null){employee = "---";} else{employee = rs.getString(3);}
                            if(rs.getString(4) == null){company = "---";} else{company = rs.getString(4);}
                            if(rs.getString(5) == null){location = "---";} else{location = rs.getString(5);}
                            if(rs.getString(6) == null){department = "---";} else{department = rs.getString(6);}
                            if(rs.getString(7) == null){cphone = "---";} else{cphone = rs.getString(7);}
                            if(rs.getString(8) == null){cdid = "---";} else{cdid = rs.getString(8);}
                            if(rs.getString(9) == null){cemail = "---";} else{cemail = rs.getString(9);}
                            if(rs.getString(10) == null){cname = "-------------";} else{cname = rs.getString(10);}
                            if(rs.getString(11) == null){sapid = "---";} else{sapid = rs.getString(11);}
                            if(rs.getString(12) == null){nextcloudu = "---";} else{nextcloudu = rs.getString(12);}
                            if(rs.getString(13) == null){ nextcloudp = "---";} else{ nextcloudp = rs.getString(13);}
                            if(rs.getString(14) == null){chostname = "--------";} else{chostname = rs.getString(14);}
                            if(rs.getString(15) == null){cmac = "-----------------";} else{cmac = rs.getString(15);}
                            if(rs.getString(16) == null){rocketu = "---";} else{rocketu = rs.getString(16);}
                            if(rs.getString(17) == null){rocketp = "---";} else{rocketp = rs.getString(17);}
                            if(rs.getString(18) == null){bitwardenu = "---";} else{bitwardenu = rs.getString(18);}
                            if(rs.getString(19) == null){bitwardenp = "---";} else{bitwardenp = rs.getString(19);}                          
                            if(rs.getString(20) == null){cip = "-----------";} else{cip = rs.getString(20);}
                            if(rs.getString(21) == null){serial = "--------";} else{serial = rs.getString(21);}
                            if(rs.getString(22) == null){pin = "------";} else{pin = rs.getString(22);}
                            if(rs.getString(23) == null){puk = "--------";} else{puk = rs.getString(23);}
                            if(rs.getString(24) == null){mkey = "-----------";} else{mkey = rs.getString(24);}
                            if(rs.getString(25) == null){duo = "-----------";} else{duo = rs.getString(25);}
                                                   
                            printComputer( cid, station, employee, company, location, department, cphone, cdid, cemail, cname, sapid, nextcloudu, nextcloudp, chostname, cmac, rocketu, rocketp, bitwardenu, bitwardenp, cip, serial, pin, puk, mkey, duo);                   
                }
            }  
    ...
    public void updateRecord(String sasDb){
        ...
        try {
            ...
            else if(selectedDb[0].equals("Computer")){
                newlog ="INSERT INTO Log (loguser, loglast, logcl, logchange, logtable) VALUES ('" + SasPass.User + "','" + llogin + "','" + clSas + "','[" + sasId + "]=> " + dataSas.toLowerCase() + "','Computers')";
            }
            else{
    ...
    public void vName(String vname) {
        ...
        if(vname.equals("DB>MAILERS") || vname.equals("DB>MA") || vname.equals("DB>MAILER") ){
            selectedDb[0] = "Mailers"; 
            System.out.println("\n\t\t>> Conected to Mailers database");
            System.out.println("\t\t===============================");
        }
        else if(vname.equals("DB>TAX") || vname.equals("DB>T") ){
            selectedDb[0] = "List";
            System.out.println("\n\t\t>> Conected to Tax database");
            System.out.println("\t\t===========================");
        }
        else if(vname.equals("DB>PASSWORD") || vname.equals("DB>PA") || vname.equals("DB>PASS") ){
            selectedDb[0] = "Pass";
            System.out.println("\n\t\t>> Conected to Password database");
            System.out.println("\t\t================================");
        }
        else if(vname.equals("DB>COMPUTER") || vname.equals("DB>PC")){
            selectedDb[0] = "Computer";
            System.out.println("\n\t\t>> Conected to Computer database");
            System.out.println("\t\t================================");
        }
        else if(vname.equals("DB>METER") || vname.equals("DB>ME") ){
            selectedDb[0] = "Meter";
            System.out.println("\n\t\t>> Conected to Meter database");
            System.out.println("\t\t=============================");
        }
        if(selectedDb[0] == "Mailers"){ 
            if(vname.equals("ALL") || vname.equals("DATE>ALL") || vname.equals("CHECK>ALL")){
                ...
                changeDateFormat();
                System.out.print("\n\t\tSent Date \tTray-1 \tTray-2 \tPieces \tSAP Status \t\t    Batch ID \n" );
                bbatch = vname.substring(6,vname.length());  
                vars("Mailers", "SELECT * FROM MAILERS WHERE [Batch]='" + bbatch + "'ORDER BY [Sent Date] ASC, ID");
                System.out.print(B);
            }
        ...
        else if(selectedDb[0] == "Computer"){
            if(vname.equals("ALL")  ){
                vars("Computer", "SELECT * FROM Computers");  
                System.out.print(B);             
            }

            else if(vname.contains("NUM>")){
                String numPass = vname.substring(4,vname.length());
                vars("Computer", "SELECT * FROM Computers WHERE ID='" + numPass + "'");
            }
            else if(vname.contains("EXT>")){
                String numPass = vname.substring(4,vname.length());
                vars("Computer", "SELECT * FROM Computers WHERE [Phone Extension]='" + numPass + "'");
            }
            else if(vname.contains("LOCATION>")){
                String numPass = vname.substring(9,vname.length());
                vars("Computer", "SELECT * FROM Computers WHERE location LIKE '%" + numPass + "%' ORDER BY [Station] ASC");
            }
            else if(vname.contains("NAME>")){
                String numPass = vname.substring(5,vname.length());
                vars("Computer", "SELECT * FROM Computers WHERE [Employee Name] LIKE '%" + numPass + "%'");
            }
        ...
        if(vname.equals("LOG>") || vname.equals("LOGVIEW>")){           
            viewLog();            
            System.out.print(B); 
        }
        System.out.print("$ ");         
